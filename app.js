...
app.use(async (req, res, next) => {
    try {
        ...
        if (req.method === 'GET' && !req.path.includes('/api/') && !req.path.includes('/auth/')) {
...
app.get('/api/bid-status/:bidId', async (req, res) => {
    try {
        ...
        const bid = await Bid.findById(req.params.bidId)
            .populate('project', 'title bidSettings');
        ...
        res.json({
            ...
            amount: bid.amount,
            project: bid.project,
            canEdit: bid.canEdit && bid.project?.bidSettings?.isActive && new Date() < bid.project.bidSettings.bidEndDate,
